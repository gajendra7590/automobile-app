<form role="form" method="POST" class="ajaxFormSubmit"
    action="{{ route('salesCash.update', ['salesCash' => $data['id']]) }}" enctype="multipart/form-data"
        ...
        <div class="row">
            <div class="form-group col-md-4">
                ...
                <input name="total_outstanding" type="number" class="form-control"
                    value="{{ isset($salesAccount['cash_outstaning_balance']) ? $salesAccount['cash_outstaning_balance'] : 0.0 }}"
                    placeholder="₹ 0.00" readonly>
            </div>
            <div class="form-group col-md-4">
                <label>Paid Amount</label>
                <input name="paid_amount" type="number" class="form-control"
                    value="{{ isset($data['debit_amount']) ? $data['debit_amount'] : 0.0 }}" placeholder="₹ 0.00">
            ...
            <div class="form-group col-md-4">
                ...
                <input name="paid_date" type="date" class="form-control"
                    value="{{ isset($data['paid_date']) ? $data['paid_date'] : date('Y-m-d') }}"
            ...
            <div class="form-group col-md-4">
                ...
                <select class="form-control" name="status">
                    <option value="1"
                        ...
                        PAID</option>
                    <option value="2"
                        {{ isset($data['paid_source']) && $data['paid_source'] == '2' ? 'selected="selected";' : '' }}>
        ...
        <div class="row">
            <div class="form-group col-md-3">
                ...
                <input name="next_due_date" type="date" class="form-control"
                    min="{{ isset($data['due_date']) ? $data['due_date'] : date('Y-m-d', strtotime('+1 day')) }}"
                    value="{{ isset($data['due_date']) ? $data['due_date'] : date('Y-m-d', strtotime('+1 day')) }}"
                    placeholder="YYYY-MM-DD">
            </div>
            <div class="form-group col-md-9">
                <label>Payment Note(If Any)</label>
                <input name="payment_note" type="text" class="form-control"
                    value="{{ isset($data['paid_note']) ? $data['paid_note'] : '' }}" placeholder="Payment Note...">
            </div>
        </div>
    </div>
    <!-- /.box-body -->
    <div class="box-footer">
        <input name="sales_account_id" type="hidden"
            value="{{ isset($salesAccount['id']) ? $salesAccount['id'] : 0 }}">
